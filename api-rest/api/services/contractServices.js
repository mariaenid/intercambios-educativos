...
export const instanceContract = contractName => new web3Service.eth.Contract(getContractArtifacts(contractName).abi)
...
export const getContractEvents = contracts =>
  contracts.reduce((
    ...
    const parameterEvents = events.map(event => {
      ...
      return { name: event.name, inputs: event.inputs, topics, contractName: contract }
    })
...
export const processLog = (log, contracts, contractEvents) => {
  ...
  if (event) {
    const result = web3Service.eth.abi.decodeLog(event.inputs, log.data, log.topic)
